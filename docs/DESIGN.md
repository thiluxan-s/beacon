...
- **Dot + connecting-rule are built as a single self-contained flex column per row** (`w-2 flex-col items-center`, dot + an absolutely-positioned `w-px` rule both centered by flexbox alignment), not a single continuous `border-l` down the `<ol>` with hand-tuned negative-`left` offsets on each dot. The border-l approach requires computing the dot's absolute offset against a containing block that's shifted by the `<li>`'s own margin — easy to get pixel-wrong and brittle to spacing changes. The flex-column version is centered by construction: flexbox guarantees the dot and the rule share the same horizontal center, and the rule's `bottom: -16px` reaches exactly into the next row's `pb-4` gap regardless of how tall any given row's message text is (single line or wrapped).
